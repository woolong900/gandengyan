 ...
 *
 * 预制体那 6 个槽其实是 2 层 × 3 张：`card_1/2/5` 是平铺的近排，`card_3/4/6` 摞在它们头上
 * （层间偏移约 +7, −17）。我们不摞，一律平铺成一排——赖子每种 4 张，一排放得下。
 *
 * 摆法按重叠方向分两种，因为两种情况里「看着齐」指的不是同一条边：
 *
 * - **左右两家**沿视线方向排，格距（23 / 35）远小于牌长（40 / 52），每张只露一条窄边，
 *   眼睛看的是整叠左右两侧的轮廓线。所以照搬预制体的**逐槽变体和逐槽坐标**：
 *   那几个变体正是为相邻两个位置各自渲染的（右家 `yqjlz1_2` 57×40 与 `yqjlz1_1` 58×42
 *   交替），原样摆上去前后两张严丝合缝；统一成一张再取平均格距，上面那张的右下角就会
 *   顶出下面那张一两像素，整叠成了锯齿。第 4 张按预制体的步长循环外推。
 * - **自家/对家**并排横排，格距（45 / 34）接近牌宽（60 / 43），重叠很浅，眼睛看的是整排
 *   的**上缘**。这时匀距 + 单张贴图最平（上缘极差 0.33 / 0.42px，混变体是 0.72 / 1.09px）。
 */
...
/** 一槽：坐标是 cocos 号，翻 y 交给 `gangSlot` */
type LzSlot = readonly [sprite: ImageName, x: number, y: number, w: number, h: number, cardX: number, cardY: number];

/**
 * 逐槽照搬预制体（左右两家）。后甩的排在数组后面、压在前一张上，正好是近的压远的。
 *
 * 牌身坐标必须一槽不动才嵌得上，但牌面字要重新排到首尾两槽连成的直线上：
...
function lzSlots(slots: ReadonlyArray<LzSlot>, cardSx: number, cardSy: number): ReadonlyArray<LaiziSlot> {
  ...
  const gy0 = first[2] + first[6];
  const gx1 = last[1] + last[5];
  const gy1 = last[2] + last[6];
  const n = slots.length - 1;
  return slots.map(([sprite, x, y, w, h], i) =>
    gangSlot(
      sprite,
      x,
      y,
...
> = {
  ...
  left: {
    ...
    slots: lzSlots(
      [
        ['zqjlz2_2', 262.95, 222.95, 72, 52, 0.1, 8.04],
        ['zqjlz_2_1', 256.17, 189.41, 73, 53, -0.12, 8.11],
        ['zqjlz1_2', 250.31, 152.22, 73, 52, -0.39, 7.91],
  ...
  right: {
    ...
    slots: lzSlots(
      [
        ['yqjlz1_2', 944.97, 601.66, 57, 40, -1.64, 7.3],
        ['yqjlz1_1', 947.98, 576.43, 58, 42, 0.35, 8.79],
        ['yqjlz1_2', 951.71, 554.77, 57, 40, -0.73, 9.03],
